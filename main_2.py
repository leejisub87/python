...
def check_buy_case(df):
    ...
    if bool(status == 'down') & bool(df.chart_name[idx-1] == 'shooting_star'):
        ...
        result = True
            ...
            df.chart_name[idx-2] == 'upper_tail_pole_yangong') & bool(df.chart_name[idx-1] == 'lower_tail_pole_yangong'):
        result = True
    elif bool(df.chart_name[idx-2]=='longbody_umbong') & bool(df.chart_name[idx-1] == 'shortbody_yangbong'):
        result = True
    elif bool(status == 'down') & bool(df.chart_name[idx-2] == 'longbody_umbong') & bool(df.chart_name[idx-1] in ['shortbody_yangbong','rickshawen_doji', 'longbody_yangbong']):
        result = True
        ...
        result = True
    elif bool(status == 'down') & bool(df.chart_name[idx-3] == 'longbody_umbong') & bool(df.chart_name[idx-2] == 'shortbody_yangbong') & bool(
            df.chart_name[idx-1] == 'longbody_yangbong'):
        result = True
    elif bool(status == 'down') & bool(df.chart_name[idx-3] == 'longbody_umbong') & bool(df.chart_name[idx-2] == 'doji_cross') & bool(
            df.chart_name[idx-1] == 'longbody_yangbong'):
        result = True
    elif bool(status == 'down') & bool(df.chart_name[idx-3] == 'longbody_umbong') & bool(df.chart_name[idx-2] == 'longbody_yangbong') & bool(
        ...
        result = True
            ...
            df.chart_name[idx-1] == 'shortbody_yangbong'):
        result = True
    elif bool(status == 'down') & bool(np.sum(df.color == 'blue') >= 4) & bool(df.chart_name[idx-2] == 'shooting_star') & bool(df.chart_name[idx-1] == 'longbody_yangbong'):
        result = True
    elif bool(status == 'down') & bool(df.chart_name[idx-2] == 'shooting_star') & bool(df.chart_name[idx-1] == 'pole_yangbong'):
        result = True
    elif bool(status == 'down') & bool(df.chart_name[idx-2] == 'longbody_umbong') & bool(df.chart_name[idx-1] in ['longbody_yangbong', 'shortbody_yangbong']):
        result = True
        ...
        result = True
    elif bool(np.sum(df.color == 'blue') >= 4) & bool(np.sum(df.chart_name == 'pole_umbong')>=2) & bool(df.chart_name[idx-1] in ['shooting_star','upper_tail_pole_umbong']) :
        result = True
    elif bool(np.sum(df.color == 'blue') >= 4) & bool(df.chart_name[idx-1] in ['doji_cross', 'spinning_tops']):
        result = True
    elif bool(status == 'down') & bool(np.sum(df.chart_name == 'longbody_umbong')>=2) & bool(np.sum(df.chart_name == 'longbody_yangbong')>=1) & bool(df.chart_name[idx-1] in ['longbody_umbong']):
        result = True
...
def check_sell_case(df):
    ...
    #### 하락 반전형 캔들
    ## 유성형
    if bool(status == 'up') & bool(df.chart_name[idx-1] == 'upper_tail_pole_yangong') & bool(df.color[idx-1] == 'blue'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-1] == 'lower_tail_pole_yangbong') & bool(df.color[idx-1] == 'red'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-1] == 'lower_tail_pole_umbong') & bool(df.color[idx-1] == 'red'):
        result = True
    elif bool(status == 'up') & (bool(df.chart_name[idx-2] in ['doji_cross', 'rickshawen_doji']) | bool(df.chart_name[idx-1] in ['doji_cross', 'rickshawen_doji'])):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-2] == 'shortbody_yangbong') & bool(df.chart_name[idx-1] == 'longbody_yangbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-3] == 'upper_tail_pole_umbong') & bool(df.chart_name[idx-2] == 'longbody_umbong') & bool(df.chart_name[idx-1]=='lower_tail_pole_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-2] == 'longbody_yangbong') & bool(df.chart_name[idx-1] == 'doji_corss'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-2] == 'longbody_yangbong') & bool(df.chart_name[idx-1] == 'upper_tail_pole_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-3] == 'longbody_yangbong') & bool(df.chart_name[idx-2] == 'longbody_umbong') & bool(df.chart_name[idx-1] == 'shortbody_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-3] == 'longbody_yangbong') & bool(df.chart_name[idx-2] == 'shortbody_umbong') & bool(df.chart_name[idx-1] == 'longbody_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-2] == 'longbody_yangbong') & bool(df.chart_name[idx-1] == 'longbody_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-2] == 'longbody_yangbong') & bool(df.chart_name[idx-1] == 'longbody_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-3] == 'longbody_yangbong') & bool(df.chart_name[idx-2] == 'shortbody_yangbong') & bool(df.chart_name[idx-1] == 'longbody_umbong'):
        result = True
#########################################
    elif bool(status == 'up') & bool(df.chart_name[idx-3] == 'longbody_yangbong') & bool(df.chart_name[idx-2] == 'doji_corss') & bool(df.chart_name[idx-1] == 'longbody_pole_umbong'):
        result = True
    elif bool(status == 'up') & bool(df.chart_name[idx-3] == 'shortbody_yangbong') & bool(df.chart_name[idx-2] == 'doji_corss') & bool(df.chart_name[idx-1] == 'shortbody_umbong'):
        result = True
...
def box_vervus(default_res):
    ...
    # 저항선 돌파 = 상승장
    up_levels = np.sum(default_df.high > max_value)
    # 지지선 파괴 = 하락장
    down_levels = np.sum(default_df.low < min_value)
    v = [0]
...
def check_buy_case2(bdf, min, max):
    ...
    # 3분봉 분석
    criteria0_1 = (temp1.color[0] == 'red')  # & (temp2.color[0] == 'red') & (temp3.color[0] == 'blue') & (temp4.color[0] == 'blue') & (temp5.color[0] == 'blue')
...
def search_coin(coin_name, interval):
    ...
    df2 = df[-7:]
    rsi = f_rsi(df)
    oscillator, coef_macd, coef_signal, coef_oscillator, min_oscillator, max_oscillator, lines_5, lines_10, lines_20 = f_oscillator(df)
    levels = buy_senario_1(df2, coin_name)

    buy_point = levels + validation.get('influence_v') \
                 + np.sum(validation.get("up_levels") > validation.get("down_levels")) \
                 + np.sum(validation.get("min_value") >= validation.get("price_currency")) \
                 + np.sum(rsi <= 28) \
                 + np.sum(np.sum([(coef_macd>0), (coef_signal>0), (coef_oscillator>0)])>=2) \
                 + np.sum(oscillator < min_oscillator) \
                 + np.sum(lines_10 < lines_5 < lines_20) \
                 + np.sum(validation.get('check_buy_2')) \
                 + np.sum(validation.get('check_buy'))
    sell_point = abs(levels) + validation.get('influence_v') \
                 + np.sum(validation.get("up_levels") < validation.get("down_levels")) \
                 + np.sum(validation.get("max_value") <= validation.get("price_currency")) \
                 + np.sum(rsi >= 72) \
                 + np.sum(np.sum([(coef_macd < 0), (coef_signal < 0), (coef_oscillator < 0)])>=3) \
                 + np.sum(oscillator > max_oscillator) \
                 + np.sum(lines_20 < lines_5 < lines_10)
    ...
    result = {'coin_name': coin_name, 'buy_point':buy_point, 'sell_point':sell_point, 'check_buy': validation.get('check_buy'),\
              'check_buy_2': validation.get('check_buy_2'), \
              'check_sell': validation.get('check_sell'), 'influence': validation.get('influence_v'),\
              'min': (validation.get("min_value") >= validation.get("price_currency")), \
              'rsi': rsi, 'macd': (np.sum([(coef_macd > 0), (coef_signal > 0), (coef_oscillator > 0)]) >= 3), \
              #'min_value':np.min([validation.get("min_value"), np.mean([lines_5, lines_20])]), 'max_value':validation.get("max_value"),'lines5_20': (lines_5 > lines_20)}
              'min_value': min_value, 'max_value': max_value,\
              'defuse1': (lines_20 < lines_5 < lines_10)}
    return result
...
def buy_coin(upbit, coin_name, ask_price, last_investment):
    ...
    print('상승 예측 / 매수 요청: ' + coin_name + "    power_buy :"+str(power_buy)+"    hoga :" + str(price) +"    count:"+str(price*count))
    try:
...
def bid_coin(upbit, coin_name, my_balances, cutoff, benefit, investment):
    #coin_name = 'KRW-SBD'

    res = search_coin(coin_name, 'minute3')
    ...
    if (0 > ratio > (-abs(cutoff))) & (res.get('buy_point') > 0) & (not res.get('defuse1')):
        type2 = "추가매수"
        influence, buy_point, sell_point = check_buy_combo(coin_name)
        if (sell_point ==0) & ((influence >= 3) | (buy_point >= 3)):
            interval = "minute10"
            print('coin_name:' + interval + '/' + coin_name + '    buy_point:' + str(buy_point) + '    influence: ' + str(
                influence))
            res = search_coin(coin_name, interval)
            print(res)
            # b = res.get('buy_point') >= 2
            # coin_name ='KRW-MVL'
            if (res.get('buy_point') >= res.get('sell_point')) & (not res.get('defuse1')):
    ...
    elif (ratio < (-abs(cutoff))) | (res.get('sell_point') >= 1): #| power_sell: # 손절
        type2 = "손절"
        try:
    ...
    elif (ratio > benefit) & (balance * avg_price >= 5500) & ((res.get('sell_point') >= 1) | (res.get('defuse1'))):
        type2 = "분할매도"
        balance_n = balance
...
def get_coin_selection(access_key, secret_key):
    ...
    while True:
        ...
        for coin_name in tickers:
            #coin_name = tickers[0]
            influence, buy_point, sell_point = check_buy_combo(coin_name)
            res = {'coin_name': coin_name, 'buy_point': buy_point, 'sell_point':sell_point, 'influence': influence}
        ...
        criteria = (df.buy_point >= 3) & (df.sell_point == 0)
        ...
        try:
            ...
            os.remove('rate/rate.json')
        except:
            pass
        if len(df) >= 1:
            merge_df(df, 'buy_selection_coin', 'buy_selection_coin.json')
            merge_df(pd.DataFrame(rate,columns=['rate'], index=[0]), 'rate', 'rate.json')
...
def execute_sell_schedule(access_key, secret_key, cutoff, benefit,investment, sec):
    upbit = pyupbit.Upbit(access_key, secret_key)
    while True:
        my_balances = []
        ...
        tickers.remove('KRW-KRW')
        tickers.remove('KRW-XYM')
        res = []
        bid_list = []
        ...
        for coin_name in tickers:
            try:
                ...
                bid_result = []
                bid_result = bid_coin(upbit, coin_name, my_balances, cutoff, benefit, investment)
...
def check_buy_combo(coin_name):
    buy_point = 0
    influence = 0
    sell_point = 0
    res = search_coin(coin_name, "minute240")
    influence = influence + res.get('influence')
    buy_point = buy_point + res.get('buy_point')
    sell_point = sell_point + res.get('sell_point')
    ...
    buy_point = buy_point + res.get('buy_point')
    sell_point = sell_point + res.get('sell_point')
    res = search_coin(coin_name, "minute10")
    influence = influence + res.get('influence')
    buy_point = buy_point + res.get('buy_point')
    sell_point = sell_point + res.get('sell_point')
    res = search_coin(coin_name, "minute3")
    ...
    buy_point = buy_point + res.get('buy_point')
    sell_point = sell_point + res.get('sell_point')
    return influence, buy_point, sell_point
...
def execute_buy_schedule(access_key, secret_key, mul, sec):
    upbit = pyupbit.Upbit(access_key, secret_key)
    while True:
        ...
        while money > 5000:
            ...
            for coin_name in tickers:
                ...
                # interval = buy_interval
                    influence, buy_point, sell_point = check_buy_combo(coin_name)
                    print('coin_name:'+coin_name+'    buy_point:'+ str(buy_point)+'    influence: '+str(influence) + '    sell_point: '+str(sell_point))
                    if (sell_point == 0) & (((influence >= 1) & (buy_point >= 1)) | (buy_point >= 2) | (influence >= 3)):
                        res = search_coin(coin_name, "minute5")
                        #b = res.get('buy_point') >= 2
                        #coin_name ='KRW-MVL'
                        print(res)
                        ask_price = round_price(res.get('min_value'))
                        last_investment = round(investment * np.max([np.min([res.get('buy_point'), mul]), 1]),-3)
                        print("last_investment: " + str(last_investment) + "   ask_price: "+str(ask_price))
                        if (res.get('buy_point') >= 1) & (res.get('sell_point') == 0):
                            ask_result = buy_coin(upbit, coin_name, ask_price, last_investment)
                        elif (res.get('buy_point') >= 2) & (res.get('sell_point') == 0) & (res.get('influence')>=1):
                            upbit.buy_market_order(coin_name, last_investment)
