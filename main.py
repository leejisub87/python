...
def excute_buy_df(df, coin_name,investment):
    status_list = []
    for i in range(len(df)):
        #i =0
        df = df[i:i + 1]
        status = 'wait'
        max_price = df.buy_price_max - 1.96 * np.sqrt(np.std(df.buy_price_max) / len(df.buy_price_max))
        updown_levels = np.mean(df.influence_m)
        volume_levels = np.mean(df.influence_v)
        ...
        if money < 5000:
            ...
            last_investment = max(investment, money)
            hoga = get_hoga_price(coin_name)
            default = max_price[0]
            criteria = max(hoga.get('ask_price')) > default > min(hoga.get('bid_price'))
            if not criteria:
                print(coin_name + "은 구매 계획에 도달하지 않았다.")
            else:
                print(coin_name + "은 구매 계획에 도달했다.")
                ho = list(hoga.get('bid_price')[hoga.get('bid_price') <= default])
                if len(ho) >= 3:
                    price_set = ho[-3:]
                else:
                    price_set = ho
...
def execute_buy_schedule(upbit, buy_df, investment):
    ...
    if money < 5000:
        ...
        for coin_name in tickers:
            ...
            df.reset_index(drop=True, inplace=True)
            status = excute_buy_df(df, coin_name,investment)
    return result
...
def save_uuid(uuid):
    ...
    df = pd.DataFrame([uuid.get('uuid')], columns=['uuid'])
    #df = df.dropna(axis=0)
    ...
    if not os.path.isfile(directory):
        ...
        df = pd.concat([ori_df.uuid, ori_df.uuid], axis=0)
    os.remove(json_file)
    # 저장
...
# 폴더생성
    ...
    #데이터 읽고 머지
    df = df[df.start_date_bid < dt_now]
    df = df[dt_now < df.end_date_bid]
    df.to_json(json_file, orient='table')
    return df
#구매 평가 요소. 주 동향,
